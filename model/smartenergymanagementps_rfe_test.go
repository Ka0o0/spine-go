...
// TC-39: Config Option A (only startTime, no endTime, no powerTimeSlot payload):
// If powerTimeSlot entries are also present → NV violation → rejected.
// Config A: powerTimeSlot NV.
func TestRFE_ConfigOptionA_SlotData_IsNV_Rejected(t *testing.T) {
	existing := makeRFEBase()
	origStart := *existing.Alternatives[0].PowerSequence[0].Schedule.StartTime

	update := rfeSeqWithKey(SmartEnergyManagementPsPowerSequenceType{
		// Config A: startTime only (no endTime)
		Schedule: &PowerSequenceScheduleDataType{
			StartTime: util.Ptr(AbsoluteOrRelativeTimeType("PT30M")),
			// EndTime intentionally absent
		},
		// NV for Config A: powerTimeSlot must NOT be present
		PowerTimeSlot: []SmartEnergyManagementPsPowerTimeSlotType{{
			Schedule: &PowerTimeSlotScheduleDataType{
				SlotNumber:      util.Ptr(PowerTimeSlotNumberType(1)),
	...
	assert.False(t, success, "Config A + powerTimeSlot (NV) must be rejected")
	assert.Equal(t, existing, result)
	assert.Equal(t, origStart, *existing.Alternatives[0].PowerSequence[0].Schedule.StartTime,
		"startTime must not be applied after rejection")
}
...
// TC-41: Config Option B (only endTime, no startTime, no powerTimeSlot):
// If powerTimeSlot entries are also present → NV violation → rejected.
// Config B: powerTimeSlot NV.
func TestRFE_ConfigOptionB_SlotData_IsNV_Rejected(t *testing.T) {
	existing := makeRFEBase()
	origEnd := *existing.Alternatives[0].PowerSequence[0].Schedule.EndTime

	update := rfeSeqWithKey(SmartEnergyManagementPsPowerSequenceType{
		Schedule: &PowerSequenceScheduleDataType{
			// StartTime intentionally absent
	...
	assert.False(t, success, "Config B + powerTimeSlot (NV) must be rejected")
	assert.Equal(t, existing, result)
	assert.Equal(t, origEnd, *existing.Alternatives[0].PowerSequence[0].Schedule.EndTime)
}
...
// TC-100: remoteWrite=true, defaultDuration below minDuration of constrained slot → rejected.
// §4.3.24.3.2.7: value must be within [minDuration, maxDuration].
func TestRFE_DefaultDuration_BelowMinDuration_Rejected(t *testing.T) {
	existing := makeRFEBase() // slot 1: min=PT10M, max=PT30M
	origDuration := *existing.Alternatives[0].PowerSequence[0].PowerTimeSlot[0].Schedule.DefaultDuration
	...
	update := rfeSeqWithKey(SmartEnergyManagementPsPowerSequenceType{
		...
		PowerTimeSlot: []SmartEnergyManagementPsPowerTimeSlotType{{
			Schedule: &PowerTimeSlotScheduleDataType{
				SlotNumber:      util.Ptr(PowerTimeSlotNumberType(1)),
				DefaultDuration: util.Ptr(DurationType("PT5M")), // below PT10M min
			},
		}},
	})
	result, success := existing.UpdateList(true, true, update, NewFilterTypePartial(), nil, nil)

	assert.False(t, success, "defaultDuration below minDuration must be rejected")
	assert.Equal(t, existing, result)
	assert.Equal(t, origDuration,
		*existing.Alternatives[0].PowerSequence[0].PowerTimeSlot[0].Schedule.DefaultDuration)
}

// TC-101: remoteWrite=true, defaultDuration above maxDuration of constrained slot → rejected.
// §4.3.24.3.2.7: value must be within [minDuration, maxDuration].
func TestRFE_DefaultDuration_AboveMaxDuration_Rejected(t *testing.T) {
	existing := makeRFEBase() // slot 1: min=PT10M, max=PT30M
	origDuration := *existing.Alternatives[0].PowerSequence[0].PowerTimeSlot[0].Schedule.DefaultDuration
	...
	update := rfeSeqWithKey(SmartEnergyManagementPsPowerSequenceType{
		...
		PowerTimeSlot: []SmartEnergyManagementPsPowerTimeSlotType{{
			Schedule: &PowerTimeSlotScheduleDataType{
				SlotNumber:      util.Ptr(PowerTimeSlotNumberType(1)),
				DefaultDuration: util.Ptr(DurationType("PT60M")), // above PT30M max
			},
		}},
	})
	result, success := existing.UpdateList(true, true, update, NewFilterTypePartial(), nil, nil)

	assert.False(t, success, "defaultDuration above maxDuration must be rejected")
	assert.Equal(t, existing, result)
	assert.Equal(t, origDuration,
		*existing.Alternatives[0].PowerSequence[0].PowerTimeSlot[0].Schedule.DefaultDuration)
}
